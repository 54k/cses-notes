...
public class Task4 {
    ...
    static class SegTree {
        ...
        void mult(int x, int xl, int xr, int l, int r, int v) {
            ...
            if (l <= xl && xr <= r) {
                tree[x].mult = opModify(tree[x].mult, v, 1);
                tree[x].sum = opModify(tree[x].sum, v, xr - xl);
                return;
            }
            ...
            tree[x].sum = opModify(opSum(tree[x * 2 + 1].sum, tree[x * 2 + 2].sum), tree[x].mult, xr - xl);
        }
        ...
        long sum(int x, int xl, int xr, int l, int r) {
            ...
            return opModify(res, tree[x].mult, Math.min(xr, r) - Math.max(xl, l));
        }
    }
